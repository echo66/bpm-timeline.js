function Formulas() {
	...
	this.exponential = function(x0, x1, y0, y1, x) {
		return y0 * Math.pow((y1 / y0), ((Math.min(x, x1) - x0) / (x1 - x0)));
	}

	this.exponential_integral = function(x0, x1, y0, y1, constant, x) {
	...
	this.exponential_integral_inverse = function(x0, x1, y0, y1, constant, y) {
		var dx = x1 - x0;
		var ry = y1 / y0;
	...
	this.linear = function(x0, x1, y0, y1, x) {
		return y0 + (y1 - y0) * ((Math.min(x, x1) - x0) / (x1 - x0));
	}

	this.linear_integral = function(x0, x1, y0, y1, constant, x) {
		// Integral of the following function: 
		// v(t) = V0 + (V1 - V0) * ((t - T0) / (T1 - T0))
	...
	this.linear_integral_inverse = function(x0, x1, y0, y1, constant, y) {
