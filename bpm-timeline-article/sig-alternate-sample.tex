...
In this paper, we present BPMTimeline, a time mapping library, providing a seamless mapping between score and performance time. To achieve this, we model tempo changes as tempo functions (a well documented subject in literature) and realize the mappings through integral and the inverse of the integral of tempo functions.
\end{abstract}
...
\begin{itemize}
	\item \textbf{No real impact on application performance:} the temporal overhead for time mappings and tempo search and time mapping should remain unnoticed;
...
Time mapping is a well document subject \cite{Dannenberg:1997,Kirke:2009,honing:time2time,wheresthebeat,honing:1993}. The main idea of a time map is to relate score time (beats) with real/performance time (seconds) through algebric manipulation of the closed form of tempo functions. A tempo function \textit{T} maps score, or performance, time to a tempo value (e.g.: $T(63\,secs) = 105.12\,BPM$). Besides tempo functions and time maps, anoher way of representing expressive tempo manipulation and timing is through time shifts \cite{honing:time2time}, a function that expresses deviations of certain events in relation to a reference score/performance time line.

The available JavaScript live coding frameworks and DAWs do not offer time maps. In frameworks like Flocking \cite{flocking:icmc2014}, tempo manipulation is achieved through manual/crisp changes, and tempo automation can achieved through the use of the \textit{setInterval} or \textit{setTimeout} methods. In Tone.js \cite{tonejs:wac2015}, tempo manipulation and automation is achieved through both manual/crisp changes and through a Tone.Signal object, which offers a similar API to AudioParam.\footnote{http://webaudio.github.io/web-audio-api/\#the-audioparam-interface} Still, Tone.js offers no time mapping facilities.


\section{Theoretical Background}

We start by describing the basic relation between time, beats and tempo in order to infer the desired mappings. For additional background, please see \cite{wheresthebeat,honing:time2time}. According to \cite{wheresthebeat}, music tempo $T$ is defined as the number of pulses/beats $b$ per $t$ time units \cite{maccallum:2010},
...
According to the previous definitions, a time map can be defined by the closed form for $T(b)$, $beatPeriod(b)$, $t(b)$ and $b(t)$. For the current implementation, we provide three default closed forms for time mapping: linear, exponential and step tempo functions, all of them inspired in AudioParam automation functions.


\subsection{Supported Closed Forms} 

In the following three subsections, we present the three closed forms for time maps included in our implementation: step, linear and exponential forms. All three closed forms share a set of terms related to tempo and time:
\begin{itemize}
...
The linear closed form, based on the definition on the formula used in AudioParam \textit{linearRampToValueAtTime} method
\begin{equation}
lin_{X_0,X_1,Y_0,Y_1}(x) = Y_0 + \Delta * (x - X_0), \,\, \Delta = \frac{Y_1-Y_0}{X_1-X_0}
\end{equation}
...
\begin{displaymath} 
sol_{1}(y) = \frac{- Y_0 + k_1}{2 * \Delta} \,\, , sol_{2}(y) = \frac{- Y_0 - k_1}{2 * \Delta}
\end{displaymath}

\begin{displaymath} k_1 = Y_{0}^2 - \frac{4}{2} * \Delta * (C - y) \end{displaymath}
...
The exponential closed form uses the exponential tempo function defined in \cite{wheresthebeat}
...
The step closed form uses a similar tempo function to the AudioParam \textit{setValueAtTime},
...
the inverse of the integral is not defined for $Y_0 = 0 \lor Y_1 = 0$.  According to the previous formulas, we define the tempo and beat period functions as

\begin{displaymath}
T(b)=step_{b_0,b_1,T_0,T_1}(b),\,\,beatPeriod(b)=step_{b_0,b_1,T_0,T_1}(b)
\end{displaymath}
...
\subsection{Beats Per Minute (BPM) definition}


$T$ is (usually\footnote{In many DAWs like Ableton Live, Logic Pro Tools, Reaper, Ardour and LMMS, music tempo is expressed in BPM. Still, it should be noted that music tempo can be expressed using, for example, italian tempo markings like \textit{Largo}, \textit{Adagietto}, \textit{Andante moderato} and so on.}) expressed in BPM. For example, $120\,bpm = \frac{120\,beats}{1\,min}$. But, usually, BPM is defined as 
...
\begin{displaymath}
t = 1\,min \Rightarrow t = 60\,secs \Rightarrow  
\end{displaymath}

\begin{displaymath}
%\frac{1}{BPM} = \frac{60\,secs}{b}
beatPeriod = \frac{60\,(secs)}{BPM} \Leftrightarrow BPM = \frac{60\,(secs)}{beatPeriod}
\end{displaymath}
...
In this section, we describe our JavaScript implementation for time maps, as defined in section 2. The current implementation has seven main features: 
\begin{itemize}
